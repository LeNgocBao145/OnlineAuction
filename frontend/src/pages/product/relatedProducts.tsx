        ...
        const fetchRelatedProducts = async () => {
            ...
            try {
                ...
                const categoryIds = product.categories
                    .map(cat => typeof cat === "object" ? cat.id : null)
                ...
                const products = await productService.getRelatedProducts(
                    product.id,
                    categoryIds,
                    10, // Fetch enough to fill the grid
                    product.id // Exclude the current product
                );
                    ...
                    <div className="w-full grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                        {relatedProducts.length > 0 ? (
                            ...
                            <div className="col-span-full py-10 flex flex-col items-center justify-center text-white/40">
                                <p className="text-lg italic">No other related products in "Incoming" or "Bidding" state.</p>
                                <p className="text-sm">Check back later for more auctions in these categories!</p>
                            </div>
                        )}
                    </div>
                )}
