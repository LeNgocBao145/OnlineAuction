...
export default function SignInForm() {
    ...
    return (
        <div className='w-1/2 h-[800px] bg-(--third) rounded-lg m-auto mt-[50px] p-4'>
            <h1 className='text-(--primary) text-center text-5xl font-bold font-inter mt-8'>Sign In</h1>
            <p className='text-white/60 text-center mt-4'>Welcome back to Online Auction!</p>
            ...
            <form onSubmit={handleSubmit(signInSubmit)} className='flex flex-col items-center justify-center gap-4 mt-8'>
                ...
                <div className='w-7/10 flex justify-between items-center'>
                    <div>
                        ...
                        <label htmlFor='rememberMe' className='text-white/60 ml-1'>Remember Me</label>
                    </div>

                    <Link to="/forgot-password" className='text-(--primary) hover:underline'>Forgot Password?</Link>
                </div>
                <div className='w-7/10 flex justify-between items-center'>
                    <button type="submit" className='w-full h-[50px] bg-(--primary) rounded-lg'>Sign In</button>
                </div>
            </form>
            ...
            <p className='text-white/60 text-center'>Don't have an account? <Link to="/signUp" className='text-(--primary) hover:underline'>Sign Up</Link></p>
        </div>
    );
}